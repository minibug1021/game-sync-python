...
class Player:
    ...
    def from_dict(cls, player_data: dict, game_sync: dict, sleeper: dict):
        ...
        player.name = player_data["alter_rom_name"]
        ...
        try:
            player.status = PlayerStatus[game_sync["status"]]
        except KeyError:
    ...
    def to_dict(self):
        ...
        player_data = {
            ...
            "last_started_at": int(now.timestamp()),
            "last_logined_at": int(now.timestamp()),
            "pdw_copied_at": now.strftime("%Y-%m-%d %H:%M:%S"),
            "pgl_copied_at": now.strftime("%Y-%m-%d %H:%M:%S"),
            "langcode": self.language_code,
            "last_up_time": now.strftime("%m/%d/%y %I:%M %p"),
            "last_up_time_strict": now.strftime("%Y-%m-%d %H:%M:%S"),
            "rom_name": extra_data.version[(self.rom_code, self.language_code)],
            "alter_rom_name": self.name,
            ...
            "last_started_at_timezone": int(now.timestamp()),
        }
